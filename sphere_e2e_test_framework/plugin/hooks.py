"""Pytest hooks for Sphere E2E Test Framework plugin."""
...
from sphere_e2e_test_framework.plugin.kiwi_hooks import (
    _filter_by_kiwi_run,
    _push_to_kiwi,
)
from sphere_e2e_test_framework.plugin.metrics import _push_metrics
...
def pytest_addoption(parser):
    ...
    group = parser.getgroup("sphere", "Sphere E2E Test Framework options")
...
def pytest_configure(config):
    ...
    config.addinivalue_line("markers", "cps: CPS application specific tests")
    config.addinivalue_line("markers", "proxy: Proxy application specific tests")
    config.addinivalue_line(
        "markers",
    ...
    config.addinivalue_line(
        "markers", "cpp: C/C++ based tests (PKCS#11 native tooling)"
    )
...
def pytest_collection_modifyitems(config, items):
    ...
    if smoke_gate and smoke_gate.active:
        from sphere_e2e_test_framework.driver.smoke_gate import reorder_smoke_first
...
def pytest_sessionstart(session):
    ...
    logger.info("=" * 60)
    logger.info("Sphere E2E Test Framework - Session Started")
    ...
    if not session.config.getoption("--skip-health-check", default=False):
        ...
        if health_cfg.get("enabled", False):
            from sphere_e2e_test_framework.driver.health_check import HealthChecker
...
def pytest_runtest_setup(item):
    ...
    if smoke_gate and smoke_gate.gate_failed:
        from sphere_e2e_test_framework.driver.smoke_gate import is_smoke_test
